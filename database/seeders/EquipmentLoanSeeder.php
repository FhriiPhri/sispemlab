...
class EquipmentLoanSeeder extends Seeder
{
    ...
    [
        ...
        'prefix' => 'AV',
        'names' => ['Proyektor', 'Kamera', 'Tripod', 'Mic Wireless'],
        'brands' => ['Epson', 'Sony', 'Canon', 'Boya'],
        'location' => 'Lab Multimedia',
    ],
    [
        'category' => 'elektronik',
        'prefix' => 'EL',
        'names' => ['Multimeter', 'Power Supply', 'Solder', 'Oscilloscope'],
        'brands' => ['Sanwa', 'GwInstek', 'Tektronix'],
        'location' => 'Lab Hardware',
    ],
    [
        'category' => 'jaringan',
        'prefix' => 'NW',
        'names' => ['Crimping Tool', 'LAN Tester', 'Switch Hub', 'Router'],
        'brands' => ['Proskit', 'TP-Link', 'Mikrotik'],
...
foreach ($baseTools as $base) {
    for ($i = 0; $i < 10; $i++) { // 10 tiap kategori = 30 total 🔥
        $name = $base['names'][array_rand($base['names'])];
        $brand = $base['brands'][array_rand($base['brands'])];
        ...
        $generatedTools->push([
            ...
            'condition_status' => collect(['baik', 'rusak-ringan', 'perlu-servis'])->random(),
            'location' => $base['location'],
            'stock_total' => rand(2, 15),
            'stock_available' => rand(1, 10),
            'description' => 'Generated tool untuk testing sistem.',
        ]);
...
$tools = $generatedTools->mapWithKeys(function (array $tool) use ($categories): array {
